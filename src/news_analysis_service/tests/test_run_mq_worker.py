...
def _make_settings() -> MagicMock:
    ...
    recv = MagicMock(name="in", durable=True, auto_delete=False,
                     exclusive=False, routing_key="in.key", prefetch_count=1)
    send = MagicMock(name="out", durable=True, auto_delete=False,
                     exclusive=False, routing_key="out.key", delivery_mode=2)
...
async def _drive_main_and_capture_callback(settings_mock, analyzer, exchange, queue):  # noqa: ANN001, ANN202
    ...
    with patch.object(worker, "load_settings", return_value=settings_mock), \
...
def test_on_message_analyzes_and_publishes() -> None:
    ...
    async def scenario() -> None:
        ...
        on_message = await _drive_main_and_capture_callback(
            settings_mock, analyzer, exchange, queue
        )
