...
from modules.logger.custom_logger import CustomFormatter
...
mcp = FastMCP(
    name="TraidingBot",
    host="0.0.0.0", # for HTTP servers
    port=8050
)


@mcp.tool()
def buyAsset(symbol: str, side: OrderSide, time_in_force: TimeInForce, qty: Optional[float] = None, notional: Optional[float] = None):
...
@mcp.tool()
def sellAsset(symbol_or_asset_id: Union[UUID, str], qty: Optional[str] = None, percentage: Optional[str] = None) -> Union[Order, Dict[str, Any]]:
    close_positions_data = ClosePositionRequest(
        qty=qty,
