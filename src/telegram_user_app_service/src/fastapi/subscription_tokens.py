...
subscription_router = APIRouter(prefix="/token", tags=[V1RouterTags.SUBSCRIPTION_SERVICE])


@subscription_router.get("/", response_model=SubscriptionToken, status_code=status.HTTP_201_CREATED)
async def create_subscription_token(
    token_service: Annotated[SubscriptionTokenService, Depends(get_subscription_token_service)],
) -> Any:
...
@subscription_router.get("/list", response_model=list[SubscriptionToken], status_code=status.HTTP_200_OK)
...
@subscription_router.get("/{username}", response_model=list[SubscriptionToken], status_code=status.HTTP_200_OK)
async def get_subscription_token_by_username(
    username: Annotated[
        str, Path(..., embed=True, title="Telegram Username", description="The Telegram username to list tokens for.")
    ],
    token_service: Annotated[SubscriptionTokenService, Depends(get_subscription_token_service)],
) -> Any:
